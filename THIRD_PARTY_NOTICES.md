...
Substantial portions of the client source are adapted from
...
Copyright 2024 Matteo Spinelli. Mellon Client is licensed under the Apache
License 2.0; the project `LICENSE` contains that license text.

This revision is an evidence-based pre-import comparison baseline selected
from repository history and file similarity. It is not asserted to be a proven
Git ancestor or necessarily the exact snapshot used for MoDiff Client's
original import. MoDiff Client has modified the adapted files. In-file
modification comments identify adapted source; retain these project-level
notices alongside generated and commentless artifacts whose formats cannot
carry those comments. New MoDiff-only files are not attributed to that baseline.
