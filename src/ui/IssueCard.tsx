...
const toneClasses: Record<IssueCardTone, string> = {
  ...
  warning: 'border-modiff-warning/70 bg-modiff-surface',
  error: 'border-modiff-invalid/70 bg-modiff-surface',
...
export function IssueCard({
  ...
  return (
    <article
      data-testid={testId}
      className={cx(
        'min-w-0 overflow-hidden rounded-modiff-compact border p-3 text-sm text-modiff-text',
        toneClasses[tone],
        className,
      )}
    >
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          {meta ? <div className="mt-1 break-words text-modiff-metadata text-modiff-subtle-text">{meta}</div> : null}
          {children ? (
            <div className="mt-2 min-w-0 overflow-hidden text-modiff-metadata leading-5 text-modiff-subtle-text">
              {children}
            </div>
          ) : null}
        </div>
