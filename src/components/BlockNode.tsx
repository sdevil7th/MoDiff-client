...
import { ListChecks, LoaderCircle, Maximize2, Minimize2, PackageOpen, Save, Settings2 } from 'lucide-react';
...
/**
 * One React Flow renderer for every composite. Catalog/source identity is
 * data, not a canvas node type; V1 User Nodes remain here only as a migration
 * path while new source-neutral instances use BlockInstanceV2.
 */
const BlockNode = memo((node: NodeProps<CustomNodeType>) => {
  ...
  return node.data.blockInstanceV2 ? <BlockNodeV2 {...node} /> : <UserBlockNode {...node} />;
});
