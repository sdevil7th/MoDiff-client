...
- `executionSpecs.ts`: strict backend recipe parsing and runtime receipt construction; Flux Schnell, Dev, and Krea text-to-image plus Flux Depth control-image use this path
...
Depth control-image is the next migrated pair: its loader, control-image source,
control generator, preview route, and `FluxControlPipeline` identity are also
backend-declared. `modelProfiles.ts` retains presentation and form defaults
only for both pairs.
