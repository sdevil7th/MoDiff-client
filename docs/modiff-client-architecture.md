...
- `executionSpecs.ts`: strict backend recipe parsing and runtime receipt construction; Flux Schnell, Dev, and Krea text-to-image use this path
...
`FluxPipeline` identity now come from the backend profile/specification, while
`modelProfiles.ts` retains presentation and form defaults only.
